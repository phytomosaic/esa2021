...
Base R already contains lots of useful functions.  Use a few basic functions to explore a dataset called `spe`.

```{r useful-functions, exercise=TRUE}
?ls           # for help, put ? in front of any function
...
We've already said that you can use functions from other packages.  But you can also "roll your own"!  Functions take the generic form `f <- function(x) { x }`.  For example:

```{r define-functions, exercise=TRUE}
# function (a silly one): pick a color from the vector by position
...
#### __*Where did this dataset originate? How do I use it?*__ {#data-origin}
...
First, define a vector of colors to use.
