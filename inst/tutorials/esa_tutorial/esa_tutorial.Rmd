...
We assume you have a basic familiarity with R.  If not, please go [here]().  At the least, you should be able to assign an object, distinguish between different objects (vector, matrix, data.frame, etc.), and index rows/columns of a matrix or data.frame.

<!-- ### Why community analysis? -->
<!-- Redundancy.  Better gradient coverage.  No need to track rare/hard-to-detect taxa.  Insight into possible *mechanisms* via traits and phylogenetic relationships. -->

### Setup

#### *How do I use the software?* {#how-setup}

### Installation

Open R and install required packages as follows.  Packages contain supplemental functions you will need.  

```{r installation, exercise=FALSE, eval=FALSE}
...
To use functions provided by packages, load them into your R environment.
...
In R, you can define your own functions of the generic form `f <- function(x) { x }`.  For example, let's define a few useful convenience functions here.

```{r convenience, exercise=FALSE, eval=FALSE}
# object: a default color vector
cv <- viridis::viridis(n=99, alpha=0.9, begin=0.2, end=0.9)

# function: maps a numeric-valued vector to colors
`colvec` <- function(x) {
     return(cv[cut(x, breaks=length(cv), include.lowest=TRUE)])
}
```
...
#### *Where did this dataset originate, and what does it mean?*

### Mafragh, Algeria vegetation data

The `veg` dataset gives information about spatial coordinates, species, environment, traits, and phylogeny for plants on a coastal plain in North Africa.  Specifically, `veg` is a list containing:

* `xy`  \tab 97 observations of 2 spatial coordinates
* `spe` \tab 97 observations of 56 plant species
* `env` \tab 347 observations of 11 soil enviro variables
* `tra` \tab 12 traits for 56 plant species
* `phy` \tab phylogeny for 56 plant species
...
A common task is to read from CSV files, e.g., `x <- read.csv('path/to/file.csv')`, but here we will load data that's already in R format.
...
```{r useful-functions, exercise=TRUE}
?ls
ls()          # list items in environment
dim(spe)      # dimensions of an object
NROW(spe)
NCOL(spe)
str(spe)
head(spe)
...
#### *How do I get my data into shape for analysis?*
...
```{r transformations-setup, exercise=TRUE}
### HIDDEN
load('veg.rda')  # assumed to be in your working dir
spe <- veg$spe   # species
env <- veg$env   # environment
...
# species
vegan::tabasco(spe, col=cv)
```

```{r viz-env, exercise=TRUE}
# environment
vegan::tabasco(env, col=cv)
```

```{r viz-tra, exercise=TRUE}
# traits
vegan::tabasco(tra, col=cv)
```
...
### Beta-diversity: dust bunny index
...
### Hill numbers and rarefaction

```{r div-spaccum, exercise=TRUE}
### High beta-diversity gives a shallow curve requiring more sites to accumulate species, low beta-diversity gives steep curve that saturates quickly with fewer sites.
plot(vegan::specaccum(spe), ylab='Species')
```
...
# cleanup 
rm(y, x, xs, mod_linear, mod_nonlinear)
```
...
Create a pairwise dissimilarity matrix `D` based on shared and unshared species among sites.
...
u <- viridis::viridis(nrow(spe), begin=0.2, end=0.9, alpha=0.9)
...
par(mfrow=c(1,3), bty='l', las=1)
plot(m1)
points(m1, pch=16, col=colvec(env$k))
plot(m2)
points(m2, pch=16, col=colvec(env$k))
plot(m3)
points(m3, pch=16, col=colvec(env$k))
```
...
grp <- cutree(cl, 4)               # group memberships

# plot the clustering tree, and the cluster groups on an ordination
par(mfrow=c(1,2), bty='l', las=1)
plot(cl, col=grp, cex=0.7)
rect.hclust(cl, 4, border=1:4)
plot(scores(m1), pch=16, col=grp)
ordicluster(m1, cl, prune=3, col=grp)
...
```{r grp-differences, exercise=TRUE}

# define four groups, based on equal number of observations
(p <- quantile(env$k, probs = seq(0, 1, by=0.25))) # quantiles
env$grp <- cut(env$k, p)                   # group memberships
table(env$grp)                             # count per group
...
# plot multivariate centroids per group, on the NMS configuration
centr <- aggregate(scores(m1),
                   by=list(k_group=env$grp),
                   mean)[-1]
plot(m1$points, pch=16, col=env$grp)
points(centr[1,], pch='+', col=1, cex=3)
points(centr[2,], pch='+', col=2, cex=3)
points(centr[3,], pch='+', col=3, cex=3)
points(centr[4,], pch='+', col=4, cex=3)

# permanova works as 'regular' ANOVA when using Euclidean distances...
# assume we're interested in species richness differences per group:
env$richness <- specnumber(spe) 
De <- vegdist(env$richness, 'euc')
adonis(De ~ env$grp, perm=999) # examine F and SS
anova(lm(x$SppRich ~ env$grp)) # expect identical F and SS
...
adonis(D ~ env$grp, permutations=customperm)
```
...
The Indicator Value (_IndVal_) is the product of a species' fidelity and specificity to a group.  See:  
...
As well as R package `indicspecies` which tests whether species might indicate _multiple_ groups.

```{r indval, exercise=TRUE}
grp <- cut(env$clay, 5)                    # define groups by real data
rnd <- sample(grp, length(grp), replace=T) # define random groups by bootstrapping
iv  <- labdsv::indval(spe, grp) # indicator species analysis
...
p.adjust(iv$pval,'holm')        # correction for multiple testing
...
#### *Are traits (of species within a site) convergent or divergent?*

```{r trait-converge, exercise=TRUE}
# Euclidean trait distance (traits already scaled 0-1)
Dt <- as.matrix(dist(tra, method='euc'))
# calculate trait standardized effect size of mean pairwise distances in sites
ses <- picante::ses.mpd(spe, Dt, null.model = 'richness', abund=F, runs=999)
# does trait ses.mpd differ between habitats?
grp_clay <- cut(env$clay, 5)
par(mfrow=c(1,2))
plot(ses$mpd.obs.z ~ env$clay, xlab='Clay fraction', ylab='Trait SES(MPD)', 
     col=ifelse(ses$mpd.obs.p < 0.05, 'red', 'black'))
abline(h=0, lty=2)
text(0.2, 1, 'Divergent')
text(0.2, -4, 'Convergent')
plot(ses$mpd.obs.z ~ grp_clay, xlab='Clay group', ylab='Trait SES(MPD)')
abline(h=0, lty=2)
anova(lm(ses$mpd.obs.z ~ grp_clay))
text(1.4, 1, 'Divergent')
text(1.4, -4, 'Convergent')
...
```{r phylo-signal, exercise=TRUE, exercise.setup = 'ordination'}
