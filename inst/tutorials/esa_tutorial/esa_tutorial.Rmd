...
### for help, put ? in front of any function, like `?ls()`
...
# function (a silly one): pick an item from the vector by position
...
euc <- dist(xy)                            # spatial distance matrix
...
```{r mantel-correlogram, exercise=TRUE, exercise.setup='mantel-orig'}
...
```{r pcnm-mapping, exercise=TRUE, out.width="97%"}
euc <- dist(xy)                 # euclidean distances between sites
pc  <- pcnm(euc)                # principal coordinates of neighbor matrices
par(mfrow=c(2,3))               # map PCNMs across the study area
v <- c(1,2,4,8,16,32)
...
We can use *weighted* PCNMs as explanatory variables in CCA to construct a multiscale ordination (MSO) variogram.  Residual variance here would indicate spatial autocorrelation; we hope for this to exhibit no distance trend since we accounted for spatial autocorrelation.

```{r pcnm-mso, exercise=TRUE}
euc <- dist(xy)                 # euclidean distances between sites
rs  <- rowSums(spe) / sum(spe)  # sites weighted by abundances 
pc  <- pcnm(dist(xy), w = rs)   # *weighted* principal coordinates of neighbor matrices
ord <- cca(spe ~ scores(pc))    # CCA: species constrained by space
msord <- mso(ord, xy)           # multiscale ordination
msoplot(msord, expl=T) # MSO variogram: spatial trend in residual var?
```
