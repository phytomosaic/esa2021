...
We assume you have a basic familiarity with R.  If not, please peruse [here](http://ecology.msu.montana.edu/labdsv/R/labs/R_ecology.html){target="_blank"}.  At the least, you should be able to assign an object, distinguish between different objects (vector, matrix, data.frame, etc.), and index rows/columns of a matrix or data.frame.

We assume you have familiarity with basic statistics (population vs sample; mean, SD, SE, CI; simple linear models), and have at least dabbled in linear algebra (matrices; linear combinations), and perhaps become suspicious that calculus may indeed exist.  If not, you could brush up on [matrices](https://courses.lumenlearning.com/boundless-algebra/chapter/introduction-to-matrices/){target="_blank"}.
...
#### __*How do I get my data into shape for analysis?*__ {#preanalysis-shape}
...
```{r transformations, exercise=TRUE, fig.asp=0.50}
### load data
spe <- veg$spe   # species
env <- veg$env   # environment
tra <- veg$tra   # traits
...
A quick way to search for _multivariate_ outliers is to use the distance/dissimilarity matrix.  A _multivariate_ outlier has unusual values for a combination of multiple variables (McCune and Grace 2002).  We can identify them as the sample unit(s) which exceed a defined number of standard deviations away from the grand mean (i.e., a large _z_-score).
...
```{r ordination-plotting-01, exercise=TRUE, fig.asp=0.3333}
...
```{r ordination-plotting-02, exercise=TRUE, fig.asp=0.3333}
...
```{r phylo-plot, exercise=TRUE, fig.asp=0.3333}
### basic plotting
...
```{r phy-div-beta, exercise=TRUE, fig.asp=0.50}
...
[Data Carpentry](https://datacarpentry.org/R-ecology-lesson/index.html){target="_blank"}  

#### Ecology in R

[Dave Roberts' labdsv pages](http://ecology.msu.montana.edu/labdsv/R/){target="_blank"}  

[Jari Oksanen's vegan introduction - PDF](https://cran.r-project.org/web/packages/vegan/vignettes/intro-vegan.pdf){target="_blank"}  

[David Zelený's community analysis](http://www.davidzeleny.net/anadat-r/doku.php?id=en:start){target="_blank"}  

[Tad Fukami's ecological statistics](https://fukamilab.github.io/BIO202/){target="_blank"}  

[Steve Kembel traits and phylogenies tutorial](http://www.kembellab.ca/r-workshop/biodivR/SK_Biodiversity_R.html){target="_blank"}

#### Community analysis R packages

[CRAN Environmetrics Task View](https://cran.r-project.org/web/views/Environmetrics.html){target="_blank"}  
[vegan](https://cran.r-project.org/web/packages/vegan/index.html){target="_blank"}  
[labdsv](https://cran.r-project.org/web/packages/labdsv/index.html){target="_blank"}  
[optpart](https://cran.r-project.org/web/packages/optpart/index.html){target="_blank"}  
[ade4](https://cran.r-project.org/web/packages/ade4/index.html){target="_blank"}  
[ecodist](https://cran.r-project.org/web/packages/ecodist/index.html){target="_blank"}  
